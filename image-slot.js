...
(() => {
  ...
  function load() {
    ...
    // Standalone/offline builds inline the sidecar as a global so the slots
    // hydrate without a fetch (which can't reach a separate .json file from a
    // bundled single-file page). Falls back to fetch when the global is absent.
    const inlined = (typeof window !== 'undefined' && window.__imageSlotsState && typeof window.__imageSlotsState === 'object') ? window.__imageSlotsState : null;
    const source = inlined ? Promise.resolve(inlined) : fetch(STATE_FILE).then((r) => (r.ok ? r.json() : null));
    loadP = source
      .then((j) => {
